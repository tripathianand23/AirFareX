...
class RealAirfareJsonAdapter:
    ...
    Only raw_fares are converted into observations.
    daily_index is retained separately as a reference series.
    """
    ...
    def parse_file(self, path: str | Path) -> pd.DataFrame:
        ...
        if not path.exists():
            raise FileNotFoundError(f"Raw airfare file not found: {path}")
        ...
        if "raw_fares" not in payload:
            raise ValueError("Missing required 'raw_fares' section.")

        raw_fares = payload["raw_fares"]

        if not isinstance(raw_fares, list):
            raise ValueError("'raw_fares' must be a list.")

        if not raw_fares:
            raise ValueError("'raw_fares' is empty.")
        ...
        for row in raw_fares:
            ...
            if pd.notna(collection_timestamp) and pd.notna(advance_days):
                travel_date = (
                    collection_timestamp.normalize()
                    + pd.Timedelta(days=int(advance_days))
                )
        ...
        if df.empty:
            raise ValueError("Adapter produced zero observations.")
